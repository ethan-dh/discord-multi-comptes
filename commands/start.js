...
const { startSelfbot, isSelfbotActive } = require("../utils/selfbotManager");

module.exports = {
  data: new SlashCommandBuilder()
    .setName("start")
    .setDescription("Démarre un compte selfbot")
    .addStringOption((option) =>
      option
        .setName("utilisateur")
        .setDescription("Nom d'utilisateur ou ID du compte à démarrer")
        .setRequired(true)
    ...
    try {
      ...
      db.get(
        ...
        async (err, selfbot) => {
          ...
          if (isSelfbotActive(selfbot.id)) {
            await interaction.editReply({
              embeds: [
                createErrorEmbed(
                  "Compte déjà actif",
                  `Le compte ${selfbot.username} est déjà actif.`
                ),
              ],
            });
            return;
          ...
          // Démarrer le selfbot
          const result = await startSelfbot(selfbot, db);

          if (result.success) {
            await interaction.editReply({
              embeds: [
                createSuccessEmbed("Compte démarré", result.message, [
                  {
                    name: "ID",
            ...
            await interaction.editReply({
              embeds: [createErrorEmbed("Erreur de démarrage", result.message)],
            });
          }
        }
      );
      ...
      await interaction.editReply({
        embeds: [
          createErrorEmbed(
            "Erreur",
            "Une erreur s'est produite lors du démarrage du compte."
          ),
        ],
      });
    }
