...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .setDescription("Retire un compte selfbot")
    .addStringOption((option) =>
      option
        .setName("utilisateur")
        .setDescription("Nom d'utilisateur ou ID du compte à retirer")
        .setRequired(true)
    ...
    try {
      ...
      db.get(
        ...
        async (err, selfbot) => {
          ...
          // Si le selfbot est actif, l'arrêter d'abord
          if (isSelfbotActive(selfbot.id)) {
            await stopSelfbot(selfbot.id, db);
          }

          // Supprimer les messages privés associés au selfbot en utilisant l'ID Discord
          db.run(
            "DELETE FROM messages_prives WHERE selfbot_id = ?",
            [selfbot.user_id],
            (err) => {
              if (err) {
                console.error(
          ...
          // Supprimer le selfbot de la base de données
          db.run(
            "DELETE FROM selfbots WHERE id = ?",
            [selfbot.id],
            async (err) => {
              if (err) {
                console.error(
              ...
              await interaction.editReply({
                embeds: [
                  createSuccessEmbed(
                    "Compte retiré",
                    `Le compte ${selfbot.username} (${selfbot.user_id}) a été retiré avec succès.`
                  ),
                ],
              });
            }
      ...
      await interaction.editReply({
        embeds: [
          createErrorEmbed(
            "Erreur",
            "Une erreur s'est produite lors du retrait du compte."
          ),
        ],
      });
    }
